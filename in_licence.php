...
if($CMSNT->site('license_key') == '' || checkLicenseKey($CMSNT->site('license_key'))['status'] != false){
    if (isset($_POST['btnSaveLicense'])) {
        ...
        if($checkKey['status'] != false){
            die('<script type="text/javascript">if(!alert("'.$checkKey['description'].'")){window.history.back().location.reload();}</script>');
        }
